...
int main() {
    ...
    std::cout << "1 input mem format is CF: " << (input.is_contiguous() ? "true" : "false") << std::endl;

    at::Tensor output = at::zeros({1, 3, 4, 2}, at::CPU(at::kInt) );
    output = output.contiguous();

    std::cout << "2 output mem format is CF: " << (output.is_contiguous() ? "true" : "false") << std::endl;
