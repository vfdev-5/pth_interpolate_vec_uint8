// perf record --call-graph dwarf -F 40 -- ./profile_vect_interp
// perf report
//
// perf stat -r 5 -o baseline.txt -- ./profile_vect_interp
//
// perf record -e branches,branch-misses ./profile_vect_interp
// perf report
...
void ImagingResampleHorizontalConvolution8u4x(
  ...
  for (const auto xx : c10::irange(xsize)) {
    ...
    sss0 = (num_channels == 3) ? _mm256_shuffle_epi8(sss0, mask_ch) : sss0;
    sss1 = (num_channels == 3) ? _mm256_shuffle_epi8(sss1, mask_ch) : sss1;
...
void ImagingResampleHorizontalConvolution8u(
  ...
  for (const auto xx : c10::irange(xsize)) {
    ...
    sss = (num_channels == 3) ? _mm_shuffle_epi8(sss, mask_ch) : sss;
...
int main(int argc, char** argv) {
    ...
    auto size = 256;
    auto t_input = at::randint(0, 256, {1, 3, size, size}, at::CPU(at::kByte)).contiguous(at::MemoryFormat::ChannelsLast);
